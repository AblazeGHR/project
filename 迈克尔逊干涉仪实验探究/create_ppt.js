...
// ─── Main ───
async function main() {
  const { FaBullseye, FaCogs, FaFlask, FaLightbulb, FaChartBar, FaExclamationTriangle, FaCheckCircle, FaSearch, FaArrowRight, FaArrowLeft } = require("react-icons/fa");
  const { MdLightbulb, MdScience, MdTimeline, MdCompareArrows } = require("react-icons/md");

  const iconTarget    = await iconToBase64Png(FaBullseye, "#" + C.accent, 256);
  const iconGear      = await iconToBase64Png(FaCogs, "#" + C.secondary, 256);
  const iconFlask     = await iconToBase64Png(FaFlask, "#" + C.secondary, 256);
  const iconIdea      = await iconToBase64Png(FaLightbulb, "#" + C.gold, 256);
  const iconChart     = await iconToBase64Png(FaChartBar, "#" + C.secondary, 256);
  const iconWarning   = await iconToBase64Png(FaExclamationTriangle, "#" + C.gold, 256);
  const iconCheck     = await iconToBase64Png(FaCheckCircle, "#" + C.accent, 256);
  const iconSearch    = await iconToBase64Png(FaSearch, "#" + C.secondary, 256);
  const iconArrowR    = await iconToBase64Png(FaArrowRight, "#" + C.accent, 256);
  const iconArrowL    = await iconToBase64Png(FaArrowLeft, "#" + C.secondary, 256);
  const iconCompare   = await iconToBase64Png(MdCompareArrows, "#" + C.secondary, 256);
  ...
  // ==================== SLIDE 3: 实验背景与目的 ====================
  {
    ...
    s.addImage({ data: iconSearch, x: MARGIN + 0.3, y: 1.55, w: 0.4, h: 0.4 });
    s.addText("研究背景", {
    ...
    s.addImage({ data: iconTarget, x: 5.6, y: 1.55, w: 0.4, h: 0.4 });
    s.addText("实验目的", {
  ...
  // ==================== SLIDE 4: 实验装置 ====================
  {
    ...
    // Left: equipment list in a card
    s.addShape("rect", {
      x: MARGIN, y: 1.35, w: 3.7, h: 3.6,
      fill: { color: C.cardBg }, shadow: makeShadow(),
    });
    s.addImage({ data: iconGear, x: MARGIN + 0.3, y: 1.55, w: 0.4, h: 0.4 });
    s.addText("实验仪器", {
    ...
    // Right: optical path schematic (as styled boxes)
    s.addShape("rect", {
    ...
    // draw a simplified optical path
    const cx = 6.2, cy = 2.8;
    // Source S
    ...
    // M1 (top, movable)
    s.addShape("rect", { x: 5.8, y: 2.7, w: 0.85, h: 0.35, fill: { color: C.accent } });
    s.addText("M₁ 可动镜", { x: 5.8, y: 2.7, w: 0.85, h: 0.35, fontSize: 8, fontFace: FONT_B, color: C.white, align: "center", valign: "middle", margin: 0 });
    // M2 (right, fixed)
    s.addShape("rect", { x: 8.1, y: 2.5, w: 0.85, h: 0.35, fill: { color: C.secondary } });
    s.addText("M₂ 固定镜", { x: 8.1, y: 2.5, w: 0.85, h: 0.35, fontSize: 8, fontFace: FONT_B, color: C.white, align: "center", valign: "middle", margin: 0 });
    // Glass plate
    s.addShape("rect", { x: 5.8, y: 3.15, w: 0.85, h: 0.2, fill: { color: C.gold }, transparency: 40 });
    s.addText("玻璃板 d,n", { x: 5.8, y: 3.15, w: 0.85, h: 0.2, fontSize: 6.5, fontFace: FONT_B, color: C.text, align: "center", valign: "middle", margin: 0 });
    // E (screen)
    s.addShape("rect", { x: 5.3, y: 3.7, w: 0.7, h: 0.3, fill: { color: "8899AA" } });
    s.addText("观察屏 E", { x: 5.3, y: 3.7, w: 0.7, h: 0.3, fontSize: 7, fontFace: FONT_B, color: C.white, align: "center", valign: "middle", margin: 0 });

    // arrows: S → G1
    s.addShape("line", { x: 6.0, y: 2.175, w: 0.1, h: 0, line: { color: C.text, width: 1.5 } });
    // G1 → M1
    s.addShape("line", { x: 6.375, y: 2.35, w: 0, h: 0.35, line: { color: C.accent, width: 1.5, dashType: "dash" } });
    // M1 → G1 back
    s.addShape("line", { x: 6.375, y: 2.7, w: 0, h: -0.35, line: { color: C.accent, width: 1.5, dashType: "dash" } });
    // G1 → G2 → M2
    s.addShape("line", { x: 6.65, y: 2.175, w: 0.15, h: 0, line: { color: C.secondary, width: 1.5 } });
    s.addShape("line", { x: 7.35, y: 2.175, w: 0.75, h: 0, line: { color: C.secondary, width: 1.5 } });
    // G1 → E
    s.addShape("line", { x: 6.375, y: 2.35, w: 0, h: 0.9, line: { color: C.text, width: 1.5, dashType: "dash" } });
    s.addShape("line", { x: 6.375, y: 3.25, w: -1.075, h: 0.6, line: { color: C.text, width: 1.5, dashType: "dash" } });

    // key annotations
    s.addText([
      { text: "半透半反分光板", options: { fontSize: 8, color: C.primary, bold: true, breakLine: true } },
      { text: "补偿板（同G₁材质厚度）", options: { fontSize: 8, color: C.primary, bold: true, breakLine: true } },
      { text: "可沿臂方向移动", options: { fontSize: 8, color: C.accent, bold: true, breakLine: true } },
      { text: "位置固定", options: { fontSize: 8, color: C.secondary, bold: true } },
    ], {
      x: 7.2, y: 3.7, w: 2.0, h: 1.2,
      fontFace: FONT_B, lineSpacingMultiple: 1.3, margin: 0,
    });
    addSlideNumber(s, 4, TOTAL);
  }

  // ==================== SLIDE 5: 实验操作与现象 (Red Laser) ====================
  {
    let s = pres.addSlide();
    s.background = { color: C.lightBg };
    ...
    // Bottom: key observation callout
    s.addShape("rect", {
    ...
    s.addText("插入玻璃板后，条纹重新变成同心圆环；将 M1 向远离分光板方向移动，可重新找到清晰平行条纹。", {
      x: MARGIN + 0.3, y: 3.7, w: 8.0, h: 0.55,
      fontSize: 13, fontFace: FONT_B, color: C.text, margin: 0,
    });
    // Arrow icon
    s.addImage({ data: iconArrowR, x: MARGIN + 7.5, y: 3.6, w: 0.35, h: 0.35 });
  ...
  // ==================== SLIDE 6: 实验操作与现象 (White Light) ====================
  {
    ...
    s.addImage({ data: iconArrowL, x: MARGIN + 7.5, y: 3.6, w: 0.35, h: 0.35 });
  ...
  // ==================== SLIDE 7: 核心原理 —— 条纹形态判据 ====================
  {
    ...
    s.addImage({ data: iconIdea, x: MARGIN + 0.3, y: 1.55, w: 0.4, h: 0.4 });
    s.addText("关键区分", {
  ...
  // ==================== SLIDE 8: 原理一 —— 红光实验（视深效应） ====================
  {
    ...
    s.addImage({ data: iconIdea, x: MARGIN + 0.3, y: 3.75, w: 0.35, h: 0.35 });
    s.addText("核心洞察", {
  ...
  // ==================== SLIDE 9: 原理二 —— 白光实验（零光程差） ====================
  {
    ...
    s.addImage({ data: iconIdea, x: MARGIN + 0.3, y: 4.05, w: 0.35, h: 0.35 });
    s.addText("核心洞察", {
  ...
  // ==================== SLIDE 10: 原理对比 ====================
  {
    ...
    s.addImage({ data: iconCompare, x: MARGIN + 0.3, y: 3.85, w: 0.35, h: 0.35 });
  ...
  // ==================== SLIDE 12: 误差分析 ====================
  {
    ...
    s.addImage({ data: iconCheck, x: MARGIN + 0.3, y: 4.47, w: 0.35, h: 0.35 });
